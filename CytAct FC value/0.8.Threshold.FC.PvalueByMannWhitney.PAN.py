...
for i in range(0, len(cancerlist)) :
    ...
    output_file1.append(open(str(threshold) + ".Cutoff.FC.Pvalue." + cancerlist[i] + ".txt", 'w'))
    output_extreme.append(open(str(threshold) + ".Cutoff.Extreme.Sites." + cancerlist[i] + ".txt", 'w'))
    ...
    for j in range(0, probe_count) :
        ...
        if(len(cytoact_high) == 0 and len(cytoact_low) != 0) :
            printline1 = site_id + "\tWhole<=%s\n" % str(threshold)
            printline2 = site_id + "\t<=%s\n" % str(threshold)
            output_file1[i].write(printline1)
            output_extreme[i].write(printline2)
            continue
