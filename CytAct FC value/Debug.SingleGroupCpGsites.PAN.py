...
for i in range(0, len(cancerlist)) :
    ...
    output_file1.append(open("SingleGroupCpGsites.txt", 'w'))
    ...
    for j in range(0, probe_count) :
        ...
        if(len(cytoact_high) == 0 or len(cytoact_low) == 0) :
            print(site_id, len(cytoact_high), len(cytoact_low))
            printline = site_id + "\t%s\t%s\n" % (str(len(cytoact_high)), str(len(cytoact_low)))
