...
for i in range(len(cancerlist)) :
    ...
    output_tumor = open(file_path + cancerlist[i] + "." + target + ".Score.Matching.2.Of.Each.Sample.tumor.txt", 'w')
    output_normal = open(file_path + cancerlist[i] + "." + target + ".Score.Matching.2.Of.Each.Sample.Normal.txt", 'w')

    header = "Sample\tMethylationBurden\tExpressionMean\n"
    ...
    for j in range(len(sample_tumor)) :
        ...
        tumor_sample_information.append({"Betavalue" : [], "Expression" : [], "MethylationBurden" : 0})

    for j in range(len(sample_normal)) :
        original_sample_normal.append(sample_normal[j])
        sample_normal[j] = sample_normal[j][:15].replace('-','')
        normal_sample_information.append({"Betavalue" : [], "Expression" : [], "MethylationBurden" : 0})
    ...
    while(True) :
        ...
        for j in range(len(tumor_line)) :
            ...
            site_value.append(float(tumor_line[j]))

        site_median = numpy.median(site_value)
        for j in range(len(tumor_line)) :
            if(tumor_line[j] == "NA") : continue
            if(float(tumor_line[j]) > site_median) : tumor_sample_information[j]["MethylationBurden"] += 1
        ...
        for j in range(len(normal_line)) :
            ...
            site_value.append(float(normal_line[j]))

            
        site_median = numpy.median(site_value)
        for j in range(len(normal_line)) :
            if(normal_line[j] == "NA") : continue
            if(float(normal_line[j]) > site_median) : normal_sample_information[j]["MethylationBurden"] += 1
    ...
    for j in range(len(sample_tumor)) :
        ...
        if(len(tumor_sample_information[j]["Expression"]) != 0) :
            printline = original_sample_tumor[j] + "\t" + str(tumor_sample_information[j]["MethylationBurden"]) + "\t" + str(tumor_sample_information[j]["Expression"][-1])
            output_tumor.write(printline + "\n")

    for j in range(len(sample_normal)) :

        if(len(normal_sample_information[j]["Expression"]) != 0) :
            printline = original_sample_normal[j] + "\t" + str(normal_sample_information[j]["MethylationBurden"]) + "\t" + str(normal_sample_information[j]["Expression"][-1])
