...
probe_separation_number = 10
probe_iteration = 0

tumor_pearson_output = open("Ready.txt", 'w'); tumor_spearman_output = open("Ready.txt", 'w')
minus_pearson_output = open("Ready.txt", 'w'); minus_spearman_output = open("Ready.txt", 'w')
FC_pearson_output = open("Ready.txt", 'w'); FC_spearman_output = open("Ready.txt", 'w')
...
def reset_betavalue() :
    ...
    for reset_x in range(0, probe_separation_number) :
        betavalue_arr.append([])
        for reset_y in range(0, len(sample_id)) : betavalue_arr[reset_x].append([None, None])
        ...
        for normal_iteration in range(0, probe_separation_number_copy) : 
            ...
            for j in range(0, len(line1)) : # sample ID의 개수만큼 반복함
                ...
                if(line1[j] != "NA" and sample_name1[j] in sample_id) : # 결측치가 아니고, sample id에 포함된 경우
                    betavalue_arr[normal_iteration][sample_id.index(sample_name1[j])][0] = float(line1[j]) # sample ID에 맞는 index에 betavalue 저장
                    
            for j in range(0, len(line2)) : # sample ID의 개수만큼 반복함
                sample_name2[j] = sample_name2[j][:15].replace('-', '') # sample ID의 형식을 통일해줌

                if(line2[j] != "NA" and sample_name2[j] in sample_id) : # 결측치가 아니고, sample id에 포함된 경우
                    betavalue_arr[normal_iteration][sample_id.index(sample_name2[j])][1] = float(line2[j]) # sample ID에 맞는 index에 betavalue 저장
                
        ##################################################################################################################################################
        
        for j in range(0, probe_separation_number_copy) : # 紐⑤뱺 probe瑜?寃 ??   
            ...
            for k in range(0, len(sample_id)) :
                ...
                if(betavalue_arr[j][k][0] != None and cytoact_arr[k] != None) :
                
                    #pancancer_beta[j] = betavalue_arr[j]
                    tumor[0].append(betavalue_arr[j][k][0]); tumor[1].append(cytoact_arr[k])
                    check_tumor = True
                    
                    if(betavalue_arr[j][k][1] != None) :
                        if(abs(betavalue_arr[j][k][0] - betavalue[j][k][1]) > 0.3) : # tumor - normal betavalue > cutoff
                            correction_minus[0].append(betavalue_arr[j][k][0]); correction_minus[1].append(cytoact_arr[k])
                            check_minus = True
                        
                        if(betavalue_arr[j][k][0] > betavalue[j][k][1]) :
                            correction_FC[0].append(betavalue_arr[j][k][0]); correction_FC[1].append(cytoact_arr[k])
                            check_FC = True
