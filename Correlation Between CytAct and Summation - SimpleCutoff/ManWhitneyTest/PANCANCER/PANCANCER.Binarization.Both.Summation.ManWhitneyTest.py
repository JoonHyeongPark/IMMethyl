...
############################################################################################################################################################################
    ...
    for j in range(len(p_threshold)) :
        for k in range(len(percentage)) :
            output_file = open("Pvalue." + str(p_threshold[j]) + ".Percentage." + str(percentage[k]) + "." + cancerlist[i] + ".Both.MeaningfulCpGsites.By.ManWhitneyTest_Binarization.Summation.txt", 'w')
