...
for i in range(0, len(cancerlist)) :
   ...
   for k in range(len(percentage)) : 
        ...
        for j in range(len(PN)) :
            ...
            input_file1 = open("Pvalue." + str(P_value) + ".Percentage." + percentage[k] + "." + cancerlist[i] + "." + PN[j] + ".MeaningfulCpGsites.By.ManWhitneyTest_Binarization.Summation.txt", 'r')
        ...
        for j in range(len(PN)) :
            ...
            matplotlib.pyplot.title(cancerlist[i] + " " + PN[j] + " " + percentage[k] + " ManWhitney-test's P-value < " + str(P_value))
...
output_file = open("Pvalue." + str(P_value) + ".PANCANCER.Correlation.CytAct.Between.Meaningful.Betavalue.Count.Of.Each.CpGsite.EveryPercentage.ManWhitneyTest.txt", 'w')
