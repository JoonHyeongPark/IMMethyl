...
for i in range(0, len(cancerlist)) :
   ...
   for k in range(len(percentage)) : 
    ...
    #        pearson_pair = stats.pearsonr(summation[j], cytact[j])
            ...
            matplotlib.pyplot.title(cancerlist[i] + " WholeSites " + PN[j] + "    Spearman Cor = %s, P = %s" % (spearman_cor, spearman_P))
...
output_file = open("WholeSites.EachCancer.Correlation.CytAct.Between.Meaningful.Betavalue.Count.Of.Each.CpGsite.EveryPercentage.Excluding.Invalid.Samples.txt", 'w')
