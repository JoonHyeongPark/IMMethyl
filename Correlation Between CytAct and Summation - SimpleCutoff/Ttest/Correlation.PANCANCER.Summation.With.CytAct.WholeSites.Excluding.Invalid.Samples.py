...
#cancerlist = ["ACC", "BLCA", "BRCA", "CESC", "CHOL", "COAD", "DLBC", "ESCA", "GBM", "HNSC", "KICH", "KIRC", "KIRP", "LGG", "LIHC", "LUAD", "LUSC", "MESO", "OV", "PAAD", "PCPG", "PRAD", "READ", "SARC", "SKCM", "STAD", "TGCT", "THCA", "THYM", "UCEC", "UCS", "UVM", "PAN"]
...
for i in range(0, len(cancerlist)) :
   ...
   for k in range(len(percentage)) : 
        ...
        for j in range(len(PN)) :
            ...
            input_file1 = open("WholeSites.Percentage." + percentage[k] + "." + cancerlist[i] + "." + PN[j] + ".Binarization.Summation.Excluding.Invalid.Samples.txt", 'r')
        ...
        for j in range(len(PN)) :
            ...
            if(len(summation[j]) < 1 / float(percentage[k])) :
                print(percentage[j] + ", " + cancerlist[i] + " " + PN[j] + " " + percentage[k] + " has less than %s samples." % str(1 / float(percentage[k])))
                correlation_table[i][k][j].append("NA")
                correlation_table[i][k][j].append("NA")
                continue

            matplotlib.pyplot.hold(False)
            matplotlib.pyplot.title(cancerlist[i] + " WholeSites " + PN[j] + " " + percentage[k])
    ...
    #        pearson_pair = stats.pearsonr(summation[j], cytact[j])
            ...
            matplotlib.pyplot.title("PAN WholeSites " + PN[j] + " Spearman Cor = %s, P = %s" % (spearman_cor, spearman_P))
            ...
            figure.savefig("WholeSites." + cancerlist[i] + "." + percentage[k] + "." + PN[j] + ".pdf")

output_file = open("WholeSites.PANCANCER.Correlation.CytAct.Between.Meaningful.Betavalue.Count.Of.Each.CpGsite.EveryPercentage.Excluding.Invalid.Samples.txt", 'w')
