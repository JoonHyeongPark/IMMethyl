...
for i in range(0, len(cancerlist)) :
   ...
   for k in range(len(percentage)) : 
        ...
        for j in range(len(PN)) :
            ...
            input_file1 = open("Pvalue." + str(P_value) + ".Percentage." + percentage[k] + "." + cancerlist[i] + "." + PN[j] + ".MeaningfulCpGsites.By.Ttest_Binarization.Summation.By.Tstat.txt", 'r')
    ...
    #        pearson_pair = stats.pearsonr(summation[j], cytact[j])
            ...
            figure.savefig("Pvalue." + str(P_value) + "." + cancerlist[i] + "." + percentage[k] + "." + PN[j] + ".Excluding.Zero.pdf")

output_file = open("Pvalue." + str(P_value) + ".PANCANCER.Correlation.CytAct.Between.Meaningful.Betavalue.Count.Of.Each.CpGsite.EveryPercentage.Excluding.Zero.txt", 'w')
